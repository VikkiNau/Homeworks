...
function showTime(appMainState){
    ...
    const tiemrText = `
        ...
        <p>${timeValue.year}/${appDate(timeValue.month)}/${appDate(timeValue.day)}</p>
        <p>${appDate(timeValue.hours)} ${(timeValue.seconds % 2 === 0)? ":" : "&nbsp;"} ${appDate(timeValue.minutes)}  
        ${(timeValue.seconds % 2 === 0)? ":" : "&nbsp;"}  ${appDate(timeValue.seconds)}</p>
    `
    timeDisplayArea.innerHTML = tiemrText;
    
    
}
function appDate(time)
{
